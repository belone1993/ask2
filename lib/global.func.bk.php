...
function cutstr($string, $length, $dot = '...') {
    ...
    $strcut = '';
    if (strtolower(ASK2_CHARSET) == 'utf-8') {
        $n = $tn = $noc = 0;
...
function judgewap(){
    	...
    	if($setting['wap_domain']!=null||$setting['wap_domain']!=''){
    		if($_SERVER['HTTP_HOST']!=$setting['wap_domain']){
	
		$self=$_SERVER['REQUEST_URI'];
		$url=$setting['wap_domain'] .str_replace('index.php?', '', $self);
		
	
define('SITE_URL', $setting['wap_domain'] );
...
function makecode($code, $width = 80, $height = 28, $quality = 3) {
    ...
    $fontfile = ASK2_ROOT . '/css/default/ninab.ttf';
