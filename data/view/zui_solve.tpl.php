...
<div class="col-sm-2 hidden-md hidden-lg">
    <div class="card border_none">
        ...
        <img data-toggle="lightbox" data-image="<?=SITE_URL?>upload/code.jpg" data-caption="扫一扫图片中的二维码"
             src="<?=SITE_URL?>upload/code.jpg" width="200px" height="200px" class="img-responsive" alt="响应式图片测试">
    </div>
...
$(".question-des").find("img").attr("data-caption","<?=$question['title']?>");
    ...
    <div class="row mod-best-a js-form">
    ...
    <div class="col-sm-12">
            ...
            <div class="hd clearfix">
                ...
                <h2 class="text-danger pull-left font-18"> <i class="icon icon-check-board font-18 "></i>满意回答</h2>
                <p class="pubtime pull-right">
                    <a class="js-report ui-report" href="javascript:;" style="visibility: hidden;">检举</a>
                    <span class="line ui-line" style="visibility: hidden;">|</span><?=$bestanswer['format_adopttime']?>
                </p>
            </div>
            <div class="bd clear">
                <div class="qa-content font-15">
...
<? } } ?>
            ...
            <div class="comment-box mt10">
                <div class="comments-hd">
                    <div class="function" id='<?=$bestanswer['id']?>'>
                        <span class="number label mar-ly-1 color-white"><a class="color-white" onclick="show_comment('<?=$bestanswer['id']?>');" href="javascript:;">评论(<?=$bestanswer['comments']?>)</a></span>
                      <span id='<?=$bestanswer['id']?>' class="dianzan label mar-ly-1"><i class="icon icon-thumbs-o-up "></i><a class="button_agree  hand mar-y-1"  ><?=$bestanswer['supports']?></a> </span>                              
                    </div>
                </div>
                <div class="comments-mod clear" style="display: none;margin-left: 40px;" id="comment_<?=$bestanswer['id']?>">
             ...
             <input type="text" placeholder="请输入评论内容，不少于2个字" class="comment-input form-control" name="content" />
              ...
              <span class="input-group-btn"><input type="button" value="评论"  class="btn btn-danger" name="submit" onclick="addcomment(<?=$bestanswer['id']?>);"/> </span>
            </div>
                  
                    </div>
                    <ul class="comments-list nav">
                        <li class="loading">
                        <img src='<?=SITE_URL?>css/default/loading.gif' align='absmiddle' />
                        &nbsp;加载中...
                        </li>
                    </ul>
                </div>
            ...
            <div id="gift_box" class="ft">
                <div class="row user-vcard clearfix">
                
                    <div class="col-sm-1 info">
                        <a target="_blank" href="<?=SITE_URL?>u-<?=$bestanswer['authorid']?>.html"><img width="35px"  height="35px" class="img-rounded" src="<?=$bestanswer['author_avartar']?>" onmouseover="pop_user_on(this, '<?=$bestanswer['authorid']?>', 'image');"  onmouseout="pop_user_out();"></a>
                    </div>
                    <div class=" col-sm-11 text">
                        <p class="name"><a target="_blank" href="<?=SITE_URL?>u-<?=$bestanswer['authorid']?>.html" onmouseover="pop_user_on(this, '<?=$bestanswer['authorid']?>', 'text');"  onmouseout="pop_user_out();"><?=$bestanswer['author']?>
                        
                        </a>
                       
                           <? if($bestanswer['signature']) { ?>         ,<?=$bestanswer['signature']?>
        <? } ?>                        </p>
                        <div class="user-info">
                            ...
                            <? if($user['grouptype']==1) { ?>                             <span class="span-line">|</span>
                             <span class="admin"><a href="<?=SITE_URL?>question/editanswer/<?=$bestanswer['id']?>.html">编辑内容</a>
                            <? } ?>                        </div>
                    </div>
                </div>
            </div>
...
<? } } ?>
        ...
        <div class="actions">
                         ...
                         <? if($user['uid']>0) { ?>                          <? if($user['uid']==$question['authorid']) { ?>          
...
<button type="button" class="btn btn-success mar-b-03" onclick="deleteq('<?=$question['id']?>')" id="delete_question">删除问题</button>
...
<? if(is_array($attentionlist)) { foreach($attentionlist as $index => $hotquestion) { ?>
                <li class="b-b-line">
                    <i class="quan"></i> <a title="<?=$hotquestion['title']?>" target="_blank"
                                            href="<?=SITE_URL?>q-<?=$hotquestion['id']?>.html">
                   <?=$hotquestion['title']?><i class="icon icon-question text-success"></i> </a>
                </li>
...
<? if(is_array($nosolvelist)) { foreach($nosolvelist as $index => $newquestion) { ?>
                <li class="b-b-line">
                    <i class="quan"></i> <a title="<?=$newquestion['title']?>" target="_blank"
                                            href="<?=SITE_URL?>q-<?=$newquestion['id']?>.html">
                   <?=$newquestion['title']?><i class="icon icon-question text-success"></i> </a>
                </li>
...
$(".qa-content img").attr("data-toggle","lightbox");
$(".qa-content img").attr("class","img-thumbnail");
//删除问题
