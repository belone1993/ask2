...
<td width="20%" align="center" class="altbg2"><?=$nav['title']?></td>
                                        <td width="10%" align="center" class="altbg2"><? if($nav['type']==1) { ?><font color="orange">站内导航</a><? } else { ?>站外导航<? } ?></td>
<td width="8%" align="center" class="altbg1"><? if($nav['target']) { ?>新窗口<? } else { ?>本窗口<? } ?></td>
<td width="7%" align="center" class="altbg2"><a href="index.php?admin_nav/available/<?=$nav['id']?>/<?=$nav['available']?><?=$setting['seo_suffix']?>"><? if($nav['available']) { ?>点击禁用<? } else { ?><font color="green">点击启用</font><? } ?></a></td>
