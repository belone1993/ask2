...
</div><? if(isset($message)) { $type=isset($type)?$type:'correctmsg';  ?><div class="alert alert-primary"><?=$message?></div><? } ?><form action="index.php?admin_question/searchquestion<?=$setting['seo_suffix']?>" method="post">
    <table class="table">
        <tbody>
            ...
            <tr>
                ...
                <td  width="200" class="altbg2">提问者:<input class="txt form-control" name="srchauthor" <? if(isset($srchauthor)) { ?>value="<?=$srchauthor?>" <? } ?>></td>
...
<div id="dialog_category" title="移动分类" style="display: none">
  ...
  <script src="<?=SITE_URL?>css/dist/lib/datetimepicker/datetimepicker.min.js" type="text/javascript"></script>
