...
class usercontrol extends base {
   ...
   function onaddxinzhi(){
    ...
    if (isset($this->post['submit'])) {
    	if($_SESSION["userid"]==null||$this->post['usersid']==null){
        		echo '非法操作!';
        	    exit();
        	}
        if($_SESSION["userid"]!=$this->post['usersid']){
        		echo '非法操作!';
        	    exit();
...
//$tag=implode(',',$tagarr);
 ...
 function oneditxinzhi(){
    ...
    if (isset($this->post['submit'])) {
    	 ...
    	 if($topic['authorid']!=$this->user['uid']){
    	 	$this->message('非法操作，你的ip已被记录');
    	 }
    	if(isset($this->setting['register_on'])&&$this->setting['register_on']=='1'){
        				if($this->user['active']!=1){
 ...
 $this->message("用户名包含特殊字符", 'user/register'); exit("shit!");
  ...
  $_SESSION["userid"]= getRandChar(56);
    ...
    function  onsendcheckmail() {
    	session_start();
        if($_SESSION["formkey"]==null||$this->post['formkey']==null){
        		echo '非法操作!';
        	    exit();
        	}
        if($_SESSION["formkey"]!=$this->post['formkey']){
        		echo '非法操作!';
        	    exit();
        	}
    	if($this->user['uid']>0){
...
setcookie("useremailcheck","OKadmin",0); //浏览器关闭 是自动失效
   ...
   //用户银行
    function onuserbank() {
    	  $this->load('userbank');
       if($this->post['submit']){
       $bankcard=htmlspecialchars($this->post['bankcard']);
      
        $this->db->query("UPDATE " . DB_TABLEPRE . "user SET bankcard='$bankcard' WHERE uid={$this->user['uid']} ");
        $this->user['bankcard']=$bankcard;
       }
        $page = max(1, intval($this->get[2]));
        $pagesize = $this->setting['list_default'];
        $startindex = ($page - 1) * $pagesize;
        $moenylist= $_ENV['userbank']->getlistbytouid($this->user['uid'],$startindex,$pagesize);
        $user_rmb= $_ENV['userbank']->getsummoneybytouid($this->user['uid']);
        $rmb=0;
        if(!isset($user_rmb)){
        	$rmb=0;
        }else{
        ...
        $rownum = $this->db->fetch_total('userbank', " touid={$this->user['uid']}");
