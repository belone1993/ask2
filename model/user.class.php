...
class usermodel {
  ...
  function get_by_openid($openid) {
    ...
    function get_active_list($start = 0, $limit = 10) {
        $userlist = array();
        $query = $this->db->query("SELECT * FROM " . DB_TABLEPRE . "user ORDER BY lastlogin DESC,credit2 DESC LIMIT $start,$limit");
        while ($user = $this->db->fetch_array($query)) {
  ...
  function adduserapi($username, $password, $email = '',$groupid=7, $uid = 0) {
        $password = md5($password);
        if ($uid) {
            ...
            $this->db->query("INSERT INTO " . DB_TABLEPRE . "user(username,password,email,regip,regtime,`lastlogin`,groupid) values ('$username','$password','$email','" . getip() . "',{$this->base->time},{$this->base->time},'$groupid')");
            $uid = $this->db->insert_id();
        }
        return $uid;
    }
