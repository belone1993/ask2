...
class sowenda {
    ...
    function init_request() {
      ...
      if (!empty($this->get[1])) {
           ...
           if( $this->get[1] == 'editemail'){
           	if($_COOKIE['useremailcheck']!=null){
           	//	exit('ddddsss111'.$_COOKIE['useremailcheck']);
           	}
           	// $expire1 = time() + 600; // 设置1分钟的有效期
                   // setcookie ("emailsend1",  'dddd', $expire1); // 设置一个名字为var_name的cookie，并制定了有效期
               // echo $_COOKIE['emailsend1'];     
           	  // echo $_COOKIE['useremailcheck'];
 //  echo '----'.$_COOKIE['emailsend'];
  //exit('ok');
           }
        }
    ...
    function load_control() {
        ...
        $controlfile = ASK2_ROOT . '/control/' . $this->get[0] . '.php';
         $isapi = ('api' == substr($this->get[0], 0, 3));
          ...
          $isapi && $controlfile = ASK2_ROOT . '/control/api/' . substr($this->get[0], 4) . '.php';
         $isapp = ('app' == substr($this->get[0], 0, 3));
          $ispccaiji = ('pccaiji' == substr($this->get[0], 0, 7));
          
           $ispccaiji && $controlfile = ASK2_ROOT . '/control/pccaiji/' . substr($this->get[0], 8) . '.php';
          $isplugin = ('plugin' == substr($this->get[0], 0, 6));
            $isplugin && $controlfile = ASK2_ROOT . '/control/plugin/' . substr($this->get[0], 7) . '.php';
          $isapp && $controlfile = ASK2_ROOT . '/control/app/' . substr($this->get[0], 4) . '.php';
    ...
    function run() {
        ...
        if (method_exists($control, $method)) {
            ...
            if ($control->checkable($regular) || $isajax ||!empty($flag)) {
                $control->$method();
            } else {
